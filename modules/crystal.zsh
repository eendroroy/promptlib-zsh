...
plib_crystal_minor_version(){
  command -v crystal > /dev/null && echo -ne "$(crystal -v | head -1 | awk -F'[ .]' '{print $3}')"
}
...
plib_crystal_patch_version(){
  command -v crystal > /dev/null && echo -ne "$(crystal -v | head -1 | awk -F'[ .]' '{print $4}')"
}
...
plib_crenv_minor_version(){
  command -v crenv > /dev/null && echo -ne "$(crenv version-name | awk -F. '{print $2}')"
}

plib_crenv_patch_version(){
  command -v crenv > /dev/null && echo -ne "$(crenv version-name | awk -F. '{print $3}')"
}
